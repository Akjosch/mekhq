...
public class MekLocation extends Part {
    ...
    public MekLocation clone() {
        MekLocation clone = new MekLocation(get(Installable.class).getMainLocation(), get(Installable.class).getUnitTonnage(),
            structureType, tsm, forQuad, sensors, lifeSupport, campaign);
    ...
    public MekLocation(int loc, double tonnage, int structureType, boolean hasTSM, boolean quad, boolean sensors, boolean lifeSupport, Campaign c) {
        super(c);
    ...
    public long getStickerPrice() {
        double unitTonnage = get(Installable.class).getUnitTonnage();
        double totalStructureCost = EquipmentType.getStructureCost(getStructureType()) * unitTonnage;
        int muscCost = isTsm() ? 16000 : 2000;
        double totalMuscleCost = muscCost * unitTonnage;
        double cost = 0.1 * (totalStructureCost + totalMuscleCost);

        if (get(Installable.class).getMainLocation() == Mech.LOC_HEAD) {
            if(sensors) {
                cost += 2000 * unitTonnage;
            }
            if(lifeSupport) {
                cost += 50000;
            }
    ...
    private boolean isArm() {
        return get(Installable.class).getMainLocation() == Mech.LOC_RARM || get(Installable.class).getMainLocation() == Mech.LOC_LARM;
    }
    ...
    public boolean isSamePartType(Part part) {
        return part instanceof MekLocation
                && get(Installable.class).getMainLocation() == ((MekLocation)part).get(Installable.class).getMainLocation()
                && get(Installable.class).getUnitTonnage() == ((MekLocation)part).get(Installable.class).getUnitTonnage()
    ...
    public MissingPart getMissingPart() {
        return new MissingMekLocation(get(Installable.class).getMainLocation(), get(Installable.class).getUnitTonnage(), structureType, tsm, forQuad, campaign);
    }
    ...
    public String getDetails() {
        ...
        Unit unit = get(Installable.class).getUnit();
        if(null != unit) {
            toReturn = get(Installable.class).getLocationName();
            if(isBlownOff()) {
                toReturn += " (Blown Off)";
            } else if(isBreached()) {
                toReturn += " (Breached)";
    ...
    public boolean isSalvaging() {
        ...
        if(get(Installable.class).getMainLocation() ==  Mech.LOC_CT) {
            return false;
        }
    ...
    private void removeHeadComponents() {
        ...
        for(Part p : get(Installable.class).getUnit().getParts()) {
