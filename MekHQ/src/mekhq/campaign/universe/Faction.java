...
public class Faction {
    ...
    public static List<String> choosableFactionCodes =
        Arrays.asList("MERC,CC,DC,FS,FWL,LA,FC,ROS,CS,WOB,FRR,SIC,MOC,MH,OA,TC,CDS,CGB,CHH,CJF,CNC,CSJ,CSV,CW,TH,RWR".split(",")); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public Faction() {
        this("???", "Unknown"); //$NON-NLS-1$ //$NON-NLS-2$
    }

    public Faction(String sname, String fname) {
        shortname = sname;
        fullname = fname;
        nameGenerator = "General"; //$NON-NLS-1$
        clan = false;
        periphery = false;
        color = Color.LIGHT_GRAY;
        startingPlanet = new ArrayList<String>(Era.E_NUM);
        startingPlanet.addAll(Collections.nCopies(Era.E_NUM, "Terra")); //$NON-NLS-1$
        altNames = new ArrayList<String>(Era.E_NUM);
        altNames.addAll(Collections.nCopies(Era.E_NUM, "")); //$NON-NLS-1$
    ...
    public boolean isEmpty() {
        return shortname.equals("ABN") // Abandoned //$NON-NLS-1$
                || shortname.equals("UND") // Undiscovered //$NON-NLS-1$
                || shortname.equals("NONE") // Unexplored //$NON-NLS-1$
                || shortname.equals("???"); // Default short name for a not initialized faction //$NON-NLS-1$
    }
    ...
    private void afterUnmarshal(Unmarshaller unmarshaller, Object parent) {
        ...
        if(altNames.size() < Era.E_NUM) {
            MekHQ.logMessage(fullname + " faction did not have a long enough altNames vector"); //$NON-NLS-1$
        }
        if(eraMods.size() < Era.E_NUM) {
            MekHQ.logMessage(fullname + " faction did not have a long enough eraMods vector"); //$NON-NLS-1$
        }
        if(startingPlanet.size() < Era.E_NUM) {
            MekHQ.logMessage(fullname + " faction did not have a long enough startingPlanet vector"); //$NON-NLS-1$
        }
        
        altNames.addAll(Collections.nCopies(Math.max(0, Era.E_NUM - altNames.size()), "")); //$NON-NLS-1$
        eraMods.addAll(Collections.nCopies(Math.max(0, Era.E_NUM - eraMods.size()), 0));
        String planet = startingPlanet.isEmpty() ? "Terra" : startingPlanet.get(startingPlanet.size() - 1); //$NON-NLS-1$
    ...
    public static Faction generateLocalFaction(Planet planet) {
        return generateLocalFaction(planet, String.format("Local faction (%s)", planet.getName(null)));
    }
    ...
    public static Faction generateLocalFaction(Planet planet, String name) {
        ...
        String baseShortName = "_LOCAL_" //$NON-NLS-1$
                + planet.getId().replaceAll("\\s+", "_").toUpperCase(Locale.ROOT); //$NON-NLS-1$ //$NON-NLS-2$
        result.shortname = baseShortName;
        synchronized(LOADING_LOCK) {
            int i = 2;
            while( factions.containsKey(result.shortname) ) {
                result.shortname = baseShortName + "_" + i; //$NON-NLS-1$
                ++ i;
            }
    ...
    public static void generateFactions() throws DOMException, ParseException {
        MekHQ.logMessage("Starting load of faction data from XML..."); //$NON-NLS-1$
        synchronized (LOADING_LOCK) {
            ...
            try {
                FileInputStream fis = new FileInputStream(MekHQ.getPreference(MekHQ.DATA_DIR) + "/universe/factions.xml"); //$NON-NLS-1$
                updateFactions(fis);
                fis.close();
            } catch (Exception ex) {
                MekHQ.logError(ex);
            ...
            Utilities.parseXMLFiles(MekHQ.getPreference(MekHQ.DATA_DIR) + "/universe/factions", //$NON-NLS-1$
                    new FileParser() {
                        @Override
            ...
            UNDISCOVERED = getFaction("UND"); //$NON-NLS-1$
            UNEXPLORED = getFaction("NONE"); //$NON-NLS-1$
            PIRATE = getFaction("PIR"); //$NON-NLS-1$
            REBELS = getFaction("REB"); //$NON-NLS-1$
        }
        MekHQ.logMessage(String.format("Loaded a total of %d factions", factions.keySet().size())); //$NON-NLS-1$
    }
