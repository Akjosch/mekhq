...
public final class StarUtil {
    ...
    public static int getSpectralClassFrom(String spectral) {
        switch(spectral.trim().toUpperCase(Locale.ROOT)) {
            case "O": return Star.SPECTRAL_O; //$NON-NLS-1$
            case "B": return Star.SPECTRAL_B; //$NON-NLS-1$
            case "A": return Star.SPECTRAL_A; //$NON-NLS-1$
            case "F": return Star.SPECTRAL_F; //$NON-NLS-1$
            case "G": return Star.SPECTRAL_G; //$NON-NLS-1$
            case "K": return Star.SPECTRAL_K; //$NON-NLS-1$
            case "M": return Star.SPECTRAL_M; //$NON-NLS-1$
            case "L": return Star.SPECTRAL_L; //$NON-NLS-1$
            case "T": return Star.SPECTRAL_T; //$NON-NLS-1$
            case "Y": return Star.SPECTRAL_Y; //$NON-NLS-1$
            default: return -1;
        }
    }
    ...
    public static String getSpectralClassName(int spectral) {
        switch(spectral) {
            case Star.SPECTRAL_O: return "O"; //$NON-NLS-1$
            case Star.SPECTRAL_B: return "B"; //$NON-NLS-1$
            case Star.SPECTRAL_A: return "A"; //$NON-NLS-1$
            case Star.SPECTRAL_F: return "F"; //$NON-NLS-1$
            case Star.SPECTRAL_G: return "G"; //$NON-NLS-1$
            case Star.SPECTRAL_K: return "K"; //$NON-NLS-1$
            case Star.SPECTRAL_M: return "M"; //$NON-NLS-1$
            case Star.SPECTRAL_L: return "L"; //$NON-NLS-1$
            case Star.SPECTRAL_T: return "T"; //$NON-NLS-1$
            case Star.SPECTRAL_Y: return "Y"; //$NON-NLS-1$
    ...
    public static String getSpectralType(Integer spectralClass, Double subtype, String luminosity) {
        ...
        if( luminosity == Star.LUM_VI ) {
            ...
            return String.format(Locale.ROOT, "%s" + subtypeFormat + "%s", //$NON-NLS-1$ //$NON-NLS-2$
                    ...
                    subtypeValue / 100.0, (null != luminosity ? luminosity : Star.LUM_V));
        }
    }
    ...
    protected static String validateLuminosity(String lc) {
        ...
        if( lc.startsWith("I/II") ) { return Star.LUM_II_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("I-II") ) { return Star.LUM_II_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("Ib/II") ) { return Star.LUM_II_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("Ib-II") ) { return Star.LUM_II_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("II/III") ) { return Star.LUM_III_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("II-III") ) { return Star.LUM_III_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("III/IV") ) { return Star.LUM_IV_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("III-IV") ) { return Star.LUM_IV_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("IV/V") ) { return Star.LUM_V_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("IV-V") ) { return Star.LUM_V_EVOLVED; } //$NON-NLS-1$
        if( lc.startsWith("III") ) { return Star.LUM_III; } //$NON-NLS-1$
        if( lc.startsWith("II") ) { return Star.LUM_II; } //$NON-NLS-1$
        if( lc.startsWith("IV") ) { return Star.LUM_IV; } //$NON-NLS-1$
        if( lc.startsWith("Ia-0") ) { return Star.LUM_0; } // Alias //$NON-NLS-1$
        if( lc.startsWith("Ia0") ) { return Star.LUM_0; } // Alias //$NON-NLS-1$
        if( lc.startsWith("Ia+") ) { return Star.LUM_0; } // Alias //$NON-NLS-1$
        if( lc.startsWith("Iab") ) { return Star.LUM_IAB; } //$NON-NLS-1$
        if( lc.startsWith("Ia") ) { return Star.LUM_IA; } //$NON-NLS-1$
        if( lc.startsWith("Ib") ) { return Star.LUM_IB; } //$NON-NLS-1$
        if( lc.startsWith("I") ) { return Star.LUM_I; } // includes Ia, Iab and Ib //$NON-NLS-1$
        if( lc.startsWith("O") ) { return Star.LUM_0; } //$NON-NLS-1$
        if( lc.startsWith("VII") ) { return Star.LUM_VII; } //$NON-NLS-1$
        if( lc.startsWith("VI+") ) { return Star.LUM_VI_PLUS; } //$NON-NLS-1$
        if( lc.startsWith("VI") ) { return Star.LUM_VI; } //$NON-NLS-1$
        if( lc.startsWith("V") ) { return Star.LUM_V; } //$NON-NLS-1$
        return null;
    }
